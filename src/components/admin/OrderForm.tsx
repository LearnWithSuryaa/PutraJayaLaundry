...
const formSchema = z.object({
  ...
  items: z.array(
    z.object({
      ...
      quantity: z.coerce.number().min(0.01, "Minimal 0.01"),
...
export function OrderForm({ open, onOpenChange, onSuccess }: OrderFormProps) {
  ...
  const runningTotal = useMemo(() => {
    return watchedItems.reduce((acc, curr) => {
      ...
      return acc + calculateRoundedPrice(price, quantity);
    }, 0);
  }, [watchedItems]);
  ...
  async function onSubmit(values: FormValues) {
    ...
    const total_price = values.items.reduce(
      (acc, curr) => acc + calculateRoundedPrice(curr.price, curr.quantity),
      0
    );
