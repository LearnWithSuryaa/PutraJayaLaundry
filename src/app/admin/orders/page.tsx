...
export default function OrdersPage() {
  ...
  const sendWhatsApp = useCallback((order: Order, type: "nota" | "status") => {
    ...
    if (type === "nota") {
      ...
      const itemsList =
        order.order_items && order.order_items.length > 0
          ? order.order_items
              .map(
                (i) =>
                  `- ${i.service_name || i.name} (${i.quantity} ${i.unit}) @ ${
                    i.service_price
                      ? formatPrice(
                          Number(i.service_price) * Number(i.quantity)
                        )
                      : "-"
                  }`
              )
      ...
      message = `*Halo Kak ${
        order.customer_name
      }!* \nTerima kasih telah mempercayakan laundry Anda di *PutraJaya Laundry*.\n\n *Detail Pesanan #${
        order.id
      }*\n${itemsList}\n\n*Total: ${formatPrice(
        order.total_price
      )}*\n *Catatan:* ${
        order.notes || "-"
      }\n\nPesanan akan segera kami proses. Kami akan kabari jika sudah selesai!`;
    } else {
